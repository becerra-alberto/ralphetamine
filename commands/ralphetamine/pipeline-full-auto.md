...
Run the complete Ralphetamine pipeline from idea to execution-ready project with **zero user interaction**. Every decision point uses structured self-evaluation: weigh risks, cons, and value — then proceed with the strongest proposal.
...
For the interactive version with user checkpoints, use `/ralphetamine:pipeline-interactive`.
...
```
Perform a premortem analysis on this Ralphetamine project.
