...
Includes a **double premortem** — a second review pass that accounts for changes made by the first — to compensate for the lack of human oversight.
...
**At every phase, maximally delegate work to sub-agents via the Task tool.** The main conversation context is a finite resource that must survive the entire 9-phase pipeline. Follow these rules:
...
1. **PRD generation fails:** Retry with simpler scope interpretation.
2. **Git commit fails:** Warn and continue.
3. **Spec sub-agent fails:** Retry that story's spec sequentially.
4. **Premortem finds critical issues:** Always fix. Never skip Premortem 2 even if Premortem 1 was clean.
5. **Run script creation fails:** Output script content inline.
