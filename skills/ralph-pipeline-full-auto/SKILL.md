---
name: ralph-pipeline-full-auto
description: "Run the complete Ralph v2 pipeline fully autonomously with zero user input: PRD creation, spec generation, double premortem review, and run script generation. Triggers on: /ralph-pipeline-full-auto, ralph autopilot, ralph full auto, ralph autonomous pipeline, ralph auto."
---
...
1. **Design Decisions** — What was auto-decided and why (this section is unique to full-auto mode)
...
6. **Non-Goals** — What this will NOT include (be generous here — narrow scope reduces failure risk)
...
1. **Evaluate the proposed breakdown** by asking:
   ...
   - **Coverage risk:** Does the breakdown cover all functional requirements in the PRD? Cross-reference FR numbers.
   - **Ordering risk:** Could any story fail because it assumes setup from a later story?
2. **Apply corrections** to the breakdown before generating specs. Do not generate specs from a breakdown you've identified problems with.
3. **Log the breakdown** (epic names, story IDs and titles) in the main context for forward reference. This is the one summary worth keeping.
...
- Is there a story that should be earlier?

#### Category 3: Acceptance Criteria Quality
- Are any criteria vague or untestable?
- Are there missing edge cases?
- Do API stories define the contract (routes, params, response shape)?
...
- Does the plan assume libraries are already installed without a setup story?
...
Where the interactive version would present findings and wait for user approval:

1. **For each finding, evaluate the fix** by reasoning:
   - **Value:** How likely is this issue to cause a real failure during Ralph execution?
   - **Risk of the fix:** Could the fix introduce new problems (e.g., splitting a story might create a dependency gap)?
   - **Cost:** How much rework does the fix require?
2. **Apply all critical fixes unconditionally.** These are issues that will almost certainly cause Ralph to fail.
3. **Apply warnings if the fix is low-risk.** Skip warning-level fixes only if the fix itself introduces complexity.
4. **Log all decisions** — what was fixed, what was skipped and why — in a `premortem-1-report.md` file in `.ralph/` for the user to review later.

**After applying fixes:** Spawn a validation sub-agent to re-verify consistency (same checks as Phase 3 validation).
...
1. Stage all modified files in `specs/`, `.ralph/stories.txt`, `.ralph/premortem-1-report.md`
...
Announce: **"Phase 6 complete — Premortem 1 fixes committed. Moving to Phase 7: Premortem 2."**
...
1. The list of spec file paths and stories.txt path (to read current state)
2. The Premortem 1 report path (`.ralph/premortem-1-report.md`) so it knows what was already changed
3. Instructions to focus on **cascading effects** of the fixes

This second premortem exists because fixes to one story can create new problems:
- Splitting a story may have left a dependency gap
- Adding a new story may have created a batch ordering conflict
- Rewriting acceptance criteria may have introduced scope overlap with another story
- Adding `depends_on` references may have created a circular dependency

### Premortem 2 Focus Areas

In addition to re-running all six categories from Premortem 1, specifically check:
...
- Did batch reassignments create a batch with too many stories (> 4 parallel stories risks merge conflicts)?
- Are newly added stories properly referenced in `stories.txt` with correct batch annotations?
- Did acceptance criteria rewrites accidentally narrow scope below what the PRD requires?

#### Category 8: Holistic Coherence
- Read the full stories.txt top to bottom — does the execution order make narrative sense?
- Would a developer reading just the story titles understand the build progression?
- Is the total story count reasonable for the PRD scope? (Heuristic: 1 story per functional requirement, +/- 30%)
- Are estimation labels consistent? (A story touching 6 files shouldn't be marked `small`)

### Autonomous Fix Protocol (same as Premortem 1)

Apply critical fixes unconditionally. Apply low-risk warning fixes. Log everything to `.ralph/premortem-2-report.md`.
...
1. Stage all modified files in `specs/`, `.ralph/stories.txt`, `.ralph/premortem-2-report.md`
2. Commit message: `ralph: premortem 2 fixes for [feature-name] (N issues resolved)`
3. If no fixes were needed, create the report file anyway (documenting the clean result) and commit it.

Announce: **"Phase 8 complete — Premortem 2 fixes committed. Moving to Phase 9: Run Script."**
...
Use these templates when spawning Task agents to ensure consistent behavior.
...
```
You are performing a premortem analysis on a Ralph v2 project plan.
